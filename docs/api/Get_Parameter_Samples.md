...
The second call (with `parameters2`) returns values from the parameter archive if the parameter archive has been enabled. It is typically much faster for large intervals. Note that as the parameter archive is backfilled (i.e. not filled in realtime), `the parameters2` call will try to return the latest values from the parameter cache. See the [Processor] configuration section on how to configure the Parameter Cache.
